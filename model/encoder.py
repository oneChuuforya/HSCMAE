...
def _get_active_ex_or_ii(H, returning_active_ex=True):
    h_repeat = H // _cur_active.shape[-1]
    active_ex = _cur_active.repeat_interleave(h_repeat, dim=2)
    return active_ex if returning_active_ex else active_ex.squeeze(1).nonzero(as_tuple=True)  # ii: bi, hi, wi  #返回非零位置坐标
...
def sp_bn_forward(self, x: torch.Tensor):  #BCHW
    ii = _get_active_ex_or_ii(H=x.shape[2], returning_active_ex=False)
    
    bhwc = x.permute(0, 2, 1)
...
class SparseMaxPooling(nn.MaxPool1d):
    forward = sp_conv_forward   # hack: override the forward function; see `sp_conv_forward` above for more details


class SparseAvgPooling(nn.AvgPool1d):
    forward = sp_conv_forward   # hack: override the forward function; see `sp_conv_forward` above for more details
...
class SparseConvNeXtLayerNorm(nn.LayerNorm):
    ...
    def forward(self, x):
        if x.ndim == 3: # BHWC or BCHW
            if self.data_format == "channels_last": # BHC
                if self.sparse:
                ...
                if self.sparse:
                    ...
                    x = x.permute(0, 2, 1)
                    c = super(SparseConvNeXtLayerNorm, self).forward(x)
                    c = c.permute(0, 2, 1)
                    return c
                    # u = x.mean(1, keepdim=True)#得到第1维的个数个均值
...
class SparseConvNeXtBlock(nn.Module):
    ...
    def __init__(self, dim,bigkernel, drop_path=0., layer_scale_init_value=1e-6, sparse=True, ks=7):
        super().__init__()
        if bigkernel:
            hiddim = 4*dim
        else:
            hiddim = dim//4
        self.pwconv1 = nn.Linear(dim, hiddim)  # ,kernel_size = 1,stride = args.wave_length,groups = indim)
        self.norm = SparseConvNeXtLayerNorm(hiddim, eps=1e-6, sparse=sparse)
        self.dwconv = nn.Conv1d(hiddim, hiddim, kernel_size=ks, padding=ks//2, groups=hiddim)
        self.act = nn.GELU()
        # self.pwconv2 = nn.Conv1d(hiddim,outdim,kernel_size = 1,stride = args.wave_length)
        self.grn = GRN(hiddim)
        self.pwconv2 = nn.Linear(hiddim, dim)
    ...
    def forward(self, x): #input BCH
        input = x
        x = x.permute(0, 2, 1)  # (N, C, H, W) -> (N, H, W, C)
        x = self.pwconv1(x)#96->96*4  BHC  128*32*336
        x = self.norm(x)
        x = x.permute(0, 2, 1)  #BCH
        x = self.dwconv(x)
        x = x.permute(0, 2, 1)  # BHC
...
class SparseEncoder(nn.Module):
    ...
    def dense_model_to_sparse(m: nn.Module, verbose=False, sbn=False):
        ...
        if isinstance(m, nn.Conv1d):
            ...
            m: nn.BatchNorm1d
            oup = (SparseSyncBatchNorm1d if sbn else SparseBatchNorm1d)(m.weight.shape[0], eps=m.eps, momentum=m.momentum, affine=m.affine, track_running_stats=m.track_running_stats)
            oup.weight.data.copy_(m.weight.data)
            oup.bias.data.copy_(m.bias.data)
